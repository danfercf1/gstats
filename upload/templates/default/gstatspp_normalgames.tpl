...
<script type="text/javascript" charset="utf-8"> 
	$(document).ready(function() {
		...
		oTable = $('#dataTable').dataTable( {
				...
				"bProcessing": false,
				"bServerSide": true,
				"sAjaxSource": "ajaxcalls.php?call=normalgames",
				...
				"fnRowCallback": function( nRow, aData, iDisplayIndex ) {
					...
					$('td:eq(0)', nRow).html("<a href=\"?gstatspp;page=normalgameplayers&gameid=" + aData[4] + "\">" + aData[0] + "</a>");
				
					$('td:eq(2)', nRow).html(toTimeHours(aData[2]));
					
					var d = new Date(aData[3] * 1000);
					$('td:eq(3)', nRow).html( padLeft(d.getFullYear(), 4, "0") + '/' + padLeft((d.getMonth() + 1), 2, "0") + '/' + padLeft(d.getDate(), 2, "0") 
...
<table cellpadding="0" cellspacing="0" border="0" class="display" id="dataTable" width="100%"> 
	<thead> 
		<tr class="dataTable_fullhead"> 
			<th width="40%" height="42">{{$lang.NormalGames.GameName}}</th> 
			<th width="20%">{{$lang.NormalGames.NumberOfPlayers}}</th> 
			<th width="20%">{{$lang.NormalGames.Duration}}</th>
			<th width="20%">{{$lang.NormalGames.EndDateTime}}</th> 
			<th></th>
		</tr> 
	</thead> 
	<tbody> 
